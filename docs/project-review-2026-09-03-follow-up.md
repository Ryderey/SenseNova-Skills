...
审查日期：2026-09-03
审查对象：当前完整工作树，`HEAD e193511`（`main`）
...
项目的主路径已经具备较好的离线回归基础：41 项测试、Ruff、仓库范围检查和 Doctor 均通过，87 个布局与 66 个风格资源完整。当前主要风险不是普遍性的代码质量问题，而是“测试全绿但契约仍可能失效”的边界问题。
...
| R03 | 已修复 | 远程图片仅接受解析到公网地址的 HTTP(S) URL；首次请求及每次重定向均重新校验 |
...
| R07 | 已修复 | Base URL 限定 HTTP(S)，拒绝 userinfo/query/fragment；未知接口协议显式失败，错误不回显 URL 凭证 |
...
最终离线验证：47 项单元测试通过；Ruff `--no-fix` 通过；Repository scope 通过；Doctor 通过。
...
| Ruff | 通过；但当前配置启用了自动修复，见 R08 |
| Repository scope | 通过 |
| Offline Doctor | 通过，未消耗模型额度 |
| 远程分支核对 | `origin` 当前仅暴露 `main`，不存在 `image-viz` |
| 工作树状态 | 审查开始时干净 |
...
- 证据：`INSTALL.md:15`、`INSTALL_CN.md:15` 使用 `git clone --branch image-viz --single-branch ...`；当前 `origin` 仅有 `main@e193511`。
- 放大因素：`skills/sn-image-base/tests/test_skill_integrity.py:174-183` 和 `skills/sn-image-doctor/scripts/check_repository_scope.py:35-37,95-103` 反而将过期命令固化为通过条件；历史台账 `docs/project-review-2026-09-03.md:4` 也仍记录 `image-viz`。
...
- 最小整改：在首次请求及每次重定向前解析目标地址，默认拒绝非公网 IPv4/IPv6、localhost 和带用户信息的 URL；如确需访问本地图片服务，提供显式、默认关闭的允许项。
- 验收：离线测试覆盖 IPv4、IPv6、DNS 解析、重定向转入私网和合法公网 URL。
...
每一项整改应同时满足：实现与文档一致、存在针对根因的离线回归测试、41 项现有测试继续通过、Ruff 以非修复模式通过、Repository scope 与 Doctor 通过，并在新的整改台账中记录对应提交和证据。
